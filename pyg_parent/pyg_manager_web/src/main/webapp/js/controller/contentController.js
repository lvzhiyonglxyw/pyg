//控制层
app.controller('contentController', function ($scope, $controller,uploadServices,contentCategoryService, contentService) {
    ...
    //文件上传
    $scope.entity = {};//初始化图片对象
    $scope.uploadFile = function () {
        uploadServices.uploadFile().success(
            function (respose) {
                if (respose.success) {
                    $scope.entity.pic = respose.message;
                }
            }
        )
    }
    ...
    $scope.contentCategoryList = function () {
        contentCategoryService.findAll().success(
            function (respose) {
                $scope.content = respose;
                for (var i = 0; i < respose.length; i++) {
                    $scope.contentList[respose[i].id] = respose[i].name;
                }
